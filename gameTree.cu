...
__device__ void nextState(BJState *state, int action, int nextC, BJState **nextP)
{
	...
	*nextP = &next;
    ...
    if (verbose)
        printf("ac=%d,nc=%d {%d,%d} sum=%d,flags=%d,bet=%d\n", state->actionTaken, next.ncards, DENOM(next.cards[0]),
            DENOM(next.cards[1]), next.sum, next.flags, next.bet);
}
...
__device__ void playHand(BJState *s,int *deck)
{
	...
	if (verbose>0) {
		...
		for (int i = 0; i < s->ncards; ++i)
			printf("%c ", strcard[DENOM(s->cards[i])]);
		printf("action %s\n", choice[action]);
	}
